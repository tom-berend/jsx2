 ...
 * @exports Geometry as Geometry.Geometry
 * @namespace
 */
...
export class Geometry {
     ...
     */
    static meetCurveCurveDiscrete(c1, c2, nr, board): Coords {
        var co: number[],
            i = Type.evaluate(nr);
     ...
     * @see Geometry._meetCurveCurveIterative
     * @see Geometry.Numerics.generalizedDampedNewton
     * @see Geometry.meetCurveCurveCobyla
     */
    static meetCurveCurveNewton(c1, c2, range1, range2, testSegment) {
        var t1, t2,
            co, r,
     ...
     * @private
     * @see Geometry.meetCurveCurveNewton
     * @see Geometry.meetCurveCurve
     */
    static _meetCurveCurveIterative(c1, c2, range1, range2, i, testSegment) {
        var ret,
            t1,// t2,
            low1, low2, up1, up2,
     ...
     * @see Geometry._meetCurveCurveRecursive
     * @see Geometry.meetCurveCurveIterative
     */
    static meetCurveCurve(c1, c2, nr, t2ini, board, method: 'newton' | 'segment' = 'segment', testSegment?): Coords | number[] {
        var co,
            zeros,
            mi1, ma1, mi2, ma2,
            i = Type.evaluate(nr);
     ...
     */
    static _bezierLineMeetSubdivision(red, blue, level, testSegment?) {
        var bbb, bbr, ar,
            r0, r1,
            m,
            p0, p1, q0, q1,
    ...
    static projectPointToTurtle(point, turtle, board) {
        ...
        for (i = 0; i < len; i++) {
            ...
            if (el.elementClass === OBJECT_CLASS.CURVE) {
                res = this.projectPointToCurve(point, el, board);
                newCoords = res[0];
                newPos = res[1];
