...
export class Metapost {
    ...
    static MP_ENDPOINT = 0
    static MP_EXPLICIT = 1
    static MP_GIVEN = 2
    static MP_CURL = 3
    static MP_OPEN = 4
    static MP_END_CYCLE = 5

    static UNITY = 1.0
    // two= 2
    // fraction_half= 0.5
    static FRACTION_ONE = 1.0
    static FRACTION_THREE = 3.0
    static ONE_EIGHTY_DEG = Math.PI
    static THREE_SIXTY_DEG = 2 * Math.PI
    // EPSILON= 1e-5
    static EPS_SQ = 1e-5 * 1e-5

    /**
     * @private
     */
    static make_choices(knots) {
        ...
        do {
            ...
            // MP 291
            if (
                p.rtype > Metapost.MP_EXPLICIT &&
                (p.x - q.x) * (p.x - q.x) + (p.y - q.y) * (p.y - q.y) < this.EPS_SQ
            ) {
                p.rtype = Metapost.MP_EXPLICIT;
                if (p.ltype === Metapost.MP_OPEN) {
                    p.ltype = Metapost.MP_CURL;
                    p.set_left_curl(this.UNITY);
                }

                q.ltype = Metapost.MP_EXPLICIT;
                if (q.rtype === Metapost.MP_OPEN) {
                    q.rtype = Metapost.MP_CURL;
        ...
        h = knots[0];
        while (endless) {
            if (h.ltype !== Metapost.MP_OPEN || h.rtype !== Metapost.MP_OPEN) {
                break;
            }
            h = h.next;
            if (h === knots[0]) {
                h.ltype = Metapost.MP_END_CYCLE;
                break;
            }
        }
        ...
        while (endless) {
            ...
            // MP 299
            q = p.next;
            if (p.rtype >= Metapost.MP_GIVEN) {
                while (q.ltype === Metapost.MP_OPEN && q.rtype === Metapost.MP_OPEN) {
                    q = q.next;
                }
                ...
                while (endless) {
                    ...
                    delta.push(Metapost.mp_pyth_add(delta_x[k], delta_y[k]));
                    if (k > 0) {
                    ...
                    if (k >= n && s.ltype !== Metapost.MP_END_CYCLE) {
                        break;
                    }
                }
                if (k === n) {
                ...
                // MP 303
                if (q.ltype === Metapost.MP_OPEN) {
                    delx = q.rx - q.x;
                    dely = q.ry - q.y;
                    if (delx * delx + dely * dely < this.EPS_SQ) {
                        q.ltype = Metapost.MP_CURL;
                        q.set_left_curl(this.UNITY);
                    } else {
                        q.ltype = Metapost.MP_GIVEN;
                        q.set_left_given(Math.atan2(dely, delx));
                    }
                }
                if (p.rtype === Metapost.MP_OPEN && p.ltype === Metapost.MP_EXPLICIT) {
                    delx = p.x - p.lx;
                    dely = p.y - p.ly;
                    if (delx * delx + dely * dely < this.EPS_SQ) {
                        p.rtype = Metapost.MP_CURL;
                        p.set_right_curl(this.UNITY);
                    } else {
                        p.rtype = Metapost.MP_GIVEN;
                        p.set_right_given(Math.atan2(dely, delx));
                    }
                }
                Metapost.mp_solve_choices(p, q, n, delta_x, delta_y, delta, psi);
            } else if (p.rtype === Metapost.MP_ENDPOINT) {
                // MP 294
                p.rx = p.x;
                p.ry = p.y;
                q.lx = q.x;
     ...
     * @private
     */
    static mp_solve_choices(p, q, n, delta_x, delta_y, delta, psi) {
        ...
        while (endless) {
            ...
            if (k === 0) {
                // MP 306
                if (s.rtype === Metapost.MP_GIVEN) {
                    // MP 314
                    if (t.ltype === Metapost.MP_GIVEN) {
                        aa = Math.atan2(delta_y[0], delta_x[0]);
                        ct_st = Metapost.mp_n_sin_cos(p.right_given() - aa);
                        ct = ct_st[0];
                        st = ct_st[1];
                        cf_sf = Metapost.mp_n_sin_cos(q.left_given() - aa);
                        cf = cf_sf[0];
                        sf = cf_sf[1];
                        Metapost.mp_set_controls(p, q, delta_x[0], delta_y[0], st, ct, -sf, cf);
                        return;
                    }
                    ...
                    uu[0] = 0;
                    ww[0] = 0;
                } else if (s.rtype === Metapost.MP_CURL) {
                    // MP 315
                    if (t.ltype === Metapost.MP_CURL) {
                        p.rtype = Metapost.MP_EXPLICIT;
                        q.ltype = Metapost.MP_EXPLICIT;
                    ...
                    uu[0] = Metapost.mp_curl_ratio(cc, rt, lt);
                    vv[0] = -psi[1] * uu[0];
                    ww[0] = 0;
                } else {
                    if (s.rtype === Metapost.MP_OPEN) {
                        uu[0] = 0;
                        vv[0] = 0;
                ...
                if (s.ltype === Metapost.MP_END_CYCLE || s.ltype === Metapost.MP_OPEN) {
                    // MP 308
                    ...
                    if (s.ltype === Metapost.MP_END_CYCLE) {
                        aa = 0;
                    ...
                    if (s.ltype === Metapost.MP_CURL) {
                        cc = s.left_curl();
                        lt = Math.abs(s.left_tension());
                        rt = Math.abs(r.right_tension());
                        ff = Metapost.mp_curl_ratio(cc, lt, rt);
                        theta[n] = -(vv[n - 1] * ff) / (this.FRACTION_ONE - ff * uu[n - 1]);
                        break;
                    }
                    if (s.ltype === Metapost.MP_GIVEN) {
        ...
        while (endless) {
            t = s.next;
            ct_st = Metapost.mp_n_sin_cos(theta[k]);
            ct = ct_st[0];
            st = ct_st[1];
            cf_sf = Metapost.mp_n_sin_cos(-psi[k + 1] - theta[k + 1]);
            cf = cf_sf[0];
            sf = cf_sf[1];
            Metapost.mp_set_controls(s, t, delta_x[k], delta_y[k], st, ct, sf, cf);
            k++;
            s = t;
            if (k === n) {
                break;
     ...
     * @private
     */
    static mp_set_controls(p, q, delta_x, delta_y, st, ct, sf, cf) {
        var rt, ss, lt, sine, rr;
        lt = Math.abs(q.left_tension());
        rt = Math.abs(p.right_tension());
        rr = Metapost.mp_velocity(st, ct, sf, cf, rt);
        ss = Metapost.mp_velocity(sf, cf, st, ct, lt);
            ...
            if ((st >= 0 && sf >= 0) || (st <= 0 && sf <= 0)) {
                ...
                if (sine > 0) {
                    ...
                    if (p.right_tension() < 0) {
                        if (Metapost.mp_ab_vs_cd(Math.abs(sf), this.FRACTION_ONE, rr, sine) < 0) {
                            rr = Math.abs(sf) / sine;
                        }
                    }
                    if (q.left_tension() < 0) {
                        if (Metapost.mp_ab_vs_cd(Math.abs(st), this.FRACTION_ONE, ss, sine) < 0) {
                            ss = Math.abs(st) / sine;
                        }
                    }
                }
     ...
     * @private
     */
    static mp_curl_ratio(gamma, a_tension, b_tension) {
     ...
     * @private
     */
    static mp_velocity(st, ct, sf, cf, t) {
        return Math.min(
            4.0,
    ...
    static makeknots(p, tension) {
        ...
        for (i = 0; i < len; i++) {
            knots.push({
                x: p[i][0],
                y: p[i][1],
                ltype: Metapost.MP_OPEN,
                rtype: Metapost.MP_OPEN,
                lx: false,
                rx: false,
                ly: tension,
                ry: tension,
     ...
     * @returns {Array}
     */
    static curve(point_list, controls) {
        var knots, len, i, ii,
            val, obj,
            isClosed = false,
            x = [],
        ...
        if (!isClosed) {
            knots[0].ltype = Metapost.MP_ENDPOINT;
            knots[0].rtype = Metapost.MP_CURL;
            knots[len - 1].rtype = Metapost.MP_ENDPOINT;
            knots[len - 1].ltype = Metapost.MP_CURL;
        }
        ...
        //                 knots[i].ltype = Metapost.MP_GIVEN;
        //             }
        //             if (val[1] !== false) {
        //                 knots[i].rx = (val[1] * Math.PI) / 180;
        //                 knots[i].rtype = Metapost.MP_GIVEN;
        //             }
        //         } else {
        //             knots[i].lx = (val * Math.PI) / 180;
        //             knots[i].rx = (val * Math.PI) / 180;
        //             knots[i].ltype = knots[i].rtype = Metapost.MP_GIVEN;
        //         }
        //     }
        // }
        ...
        //         if (parseInt(i, 10) === 0) {
        //             knots[i].rtype = Metapost.MP_CURL;
        //             knots[i].set_right_curl(val);
        //         } else if (parseInt(i, 10) === len - 1) {
        //             knots[i].ltype = Metapost.MP_CURL;
