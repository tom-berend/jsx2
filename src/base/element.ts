 ...
 * @constructor
 * @param {JXG2.Board} board Reference to the board the element is constructed on.
 * @param {Object} attributes Hash of attributes and their values.
 * @param {Number} type Element type (a <tt>JXG2.OBJECT_TYPE_</tt> value).
 * @param {Number} oclass The element's class (a <tt>JXG2.OBJECT_CLASS_</tt> value).
...
export abstract class GeometryElement extends Events {
    ...
    */
     ...
     * relationships of GeometryElements in a GROUP.  Possibly used if the visProp is 'inherit'.
     * the
     * @type Object
     */
    ...
    animationCallback
    visPropOld
    visPropCache: VisPropCache = { visible: false, strokewidth: 0, color: '', opacity: 0, point1: { x: 0, y: 0, z: 0 }, point2: { x: 0, y: 0, z: 0 }, other: {} }
    ...
    evalVisProp(key: string) {
        ...
        if (key.indexOf('.') === -1) {
            // e.g. 'visible'
            if (!(key in this.visProp)) { throw new Error(`Unknown VisProp '${key}'`) }
            ...
            if (arr.length == 2 && Type.exists(this.visProp[arr[0]])) {
                if (!(arr[1] in (this.visProp[arr[0]]))) { throw new Error(`Unknown VisProp '${key}'`) }
                val = this.visProp[arr[0]][arr[1]]
            } else if (arr.length == 3 && Type.exists(this.visProp[arr[0]]) && Type.exists(this.visProp[arr[0]][arr[1]])) {
                if (!(arr[2] in this.visProp[arr[0]][arr[1]])) { throw new Error(`Unknown VisProp '${key}'`) }
