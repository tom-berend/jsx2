...
export class Ticks extends GeometryElement {
    ...
    hasPoint(x, y) {
        ...
        if (
            ...
            this.line.otype !== OBJECT_TYPE.AXIS
        ) {
            return false;
        }
    ...
    getZeroCoordinates() {
        ...
        if (this.line.elementClass === OBJECT_CLASS.LINE) {
            if (this.line.otype === OBJECT_TYPE.AXIS) {
    ...
    //          * @private
    //          */
    //         adjustTickDistance(ticksDelta, coordsZero, deltas) {
    //             var nx,
    //                 ny,
    //                 // bounds,
    //                 distScr,
     ...
     */
    beautifyScientificNotationLabel(labelText) {
        var returnString;
    ...
    updateRendererLabels() {
        ...
        for (i = 0, j = 0; i < lenData; i++) {
            ...
            if (j < lenLabels) {
                ...
                label = createText(this.board, [ld.x, ld.y, ld.t], attr);
                this.addChild(label);
...
export function createTicks (board:Board, parents, attributes) {
    ...
    if (
        ...
        el = new Ticks(parents[0], dist, attr);
    } else {
        throw new Error(
 ...
 *
 */
export function createHatchmark(board, parents, attributes) {
