 ...
 * @constructor
 */
export class Group extends CoordsElement {

    /** the objects attached to this group */
    objects: LooseObject
    ...
    /**
      ...
      * @private
      * @type Object
      * @see JXG2.Group#_updateCoordsCache
      */
    groupCoords: LooseObject = {};
    groupNeedsRegularUpdate = true
    ...
    constructor(board: Board, id, name, objects, attributes) {
        super(board, COORDS_BY.USER, [0, 0], attributes, OBJECT_TYPE.POINT, OBJECT_CLASS.POINT)
        ...
        this.visProp = Type.initVisProps(Options.group,attributes)
        this.groupNeedsRegularUpdate = this.evalVisProp('needsregularupdate')
     ...
     * @returns {JXG2.Group} returns this (empty) group
     */
    ungroup() {
        var el, p, i;
    ...
    _updateCoordsCache(el) {
        ...
        if (el !== "" && Type.exists(this.objects[el])) {
            ...
            this.groupCoords[obj.id] = { usrCoords: obj.coords.usrCoords.slice(0) };
        }
    }
     ...
     * @returns {JXG2.Group} returns this group
     */
    update() {
        var i, drag, el,
            actionCenter,
        ...
        if (drag.action === 'translation') {
            t = [
                obj.coords.usrCoords[1] - this.groupCoords[drag.id].usrCoords[1],
                obj.coords.usrCoords[2] - this.groupCoords[drag.id].usrCoords[2]
            ];
            ...
            if (Type.exists(this[actionCenter].coords)) {
                ...
                Env.debug('Group.update: No valid center for this transformation, get out of here.');
                return this;
            }
    ...
    */
    _update_centroid_center() {
        ...
        center = [0, 0];
        len = 0;
        for (el in this.groupCoords) {
            if (this.groupCoords.hasOwnProperty(el)) {
                center[0] += this.groupCoords[el].usrCoords[1];
                center[1] += this.groupCoords[el].usrCoords[2];
                ++len;
            }
        }
        if (len > 0) {
    ...
    */
                            ...
                            if (!Type.isInArray(drag.changed, obj.id)) {
                                if (obj.elementClass === OBJECT_CLASS.POINT) {
                                    obj.coords.setCoordinates(COORDS_BY.USER, [
                                        this.groupCoords[el].usrCoords[1] + t[0],
                                        this.groupCoords[el].usrCoords[2] + t[1]
                                    ]);
                                }
                            }
                        } else if (
     ...
     * @param {JXG2.Point} object The point added to the group.
     * @returns {JXG2.Group} returns this group
     */
    addPoint(object:GeometryElement) {
     ...
     * @param {JXG2.Group} group The group added to this group.
     * @returns {JXG2.Group} returns this group
     */
    addGroup(group) {
        var el;
     ...
     * @param {JXG2.Point|String|Array|Function} object A point which will be the center of rotation, the string "centroid", or
     ...
     * the rotation center of the group {@see JXG2.Group#setRotationCenter}.
     * @param {Array|JXG2.Point} objects Array of {@link JXG2.Point} or arbitrary number of {@link JXG2.Point} elements.
     * @returns {JXG2.Group} returns this group
     */
     ...
     * the rotation center of the group {@see JXG2.Group#setRotationCenter}.
     * @param {JXG2.Point} point {@link JXG2.Point} element.
     * @returns {JXG2.Group} returns this group
     */
     ...
     * @param {JXG2.Point|String} object A point which will be the center of scaling, the string "centroid", or
     * an array of length two, or a function returning an array of length two.
     * @returns {JXG2.Group} returns this group
     */
    ...
    /**
     * Generic method for {@link JXG2.Group@setTranslationPoints} and {@link JXG2.Group@setRotationPoints}
     * @private
     */
    ...
    /**
     * Generic method for {@link JXG2.Group@addTranslationPoint} and {@link JXG2.Group@addRotationPoint}
     * @private
     */
    ...
    /**
     * Generic method for {@link JXG2.Group@removeTranslationPoint} and {@link JXG2.Group@removeRotationPoint}
     * @private
     */
     ...
     */
    setProperty() {
        Env.deprecated("Group.setProperty", "Group.setAttribute()");
 ...
 * @constructor
 * @type JXG2.Group
 * @param {JXG2.Board} board The board the points are on.
 ...
 *  (function () {
 *  var board, p, col, g;
 *  board = JXG2.JSXGraph.initBoard('JXGa2204533-db91-4af9-b720-70394de4d367', {boundingbox:[-5,5,5,-5], keepaspectratio:true, axis:true, showcopyright: false});
 *  col = 'blue';
 *  p = [];
 *  p.push(board.create('point',[-2, -1 ], {size: 5, strokeColor:col, fillColor:col}));
 *  p.push(board.create('point',[2, -1 ], {size: 5, strokeColor:col, fillColor:col}));
 ...
 *  board = JXG2.JSXGraph.initBoard('JXG781b5564-a671-4327-81c6-de915c8f924e', {boundingbox:[-5,5,5,-5], keepaspectratio:true, axis:true, showcopyright: false});
 *  col = 'blue';
 *  p = [];
 *  p.push(board.create('point',[-2, -1 ], {size: 5, strokeColor:col, fillColor:col}));
 *  p.push(board.create('point',[2, -1 ], {size: 5, strokeColor:col, fillColor:col}));
 ...
 *  (function () {
 *  var board, p, col, pol, g;
 *  board = JXG2.JSXGraph.initBoard('JXGf0491b62-b377-42cb-b55c-4ef5374b39fc', {boundingbox:[-5,5,5,-5], keepaspectratio:true, axis:true, showcopyright: false});
 *  col = 'blue';
 *  p = [];
 *  p.push(board.create('point',[-2, -1 ], {size: 5, strokeColor:col, fillColor:col}));
 *  p.push(board.create('point',[2, -1 ], {size: 5, strokeColor:'red', fillColor:'red'}));
 ...
 *  (function () {
 *  var board, p, col, pol, g;
 *  board = JXG2.JSXGraph.initBoard('JXG8785b099-a75e-4769-bfd8-47dd4376fe27', {boundingbox:[-5,5,5,-5], keepaspectratio:true, axis:true, showcopyright: false});
 *  col = 'blue';
 *  p = [];
 *  p.push(board.create('point',[-2, -1 ], {size: 5, strokeColor:col, fillColor:col}));
 *  p.push(board.create('point',[2, -1 ], {size: 5, strokeColor:'red', fillColor:'red'}));
 ...
 *  (function () {
 *  var board, p, col, pol, g;
 *  board = JXG2.JSXGraph.initBoard('JXGc3ca436b-e4fc-4de5-bab4-09790140c675', {boundingbox:[-5,5,5,-5], keepaspectratio:true, axis:true, showcopyright: false});
 *  col = 'blue';
 *  p = [];
 *  p.push(board.create('point',[-2, -1 ], {size: 5, strokeColor:col, fillColor:col}));
 *  p.push(board.create('point',[2, -1 ], {size: 5, strokeColor:'yellow', fillColor:'yellow'}));
 ...
 *  board = JXG2.JSXGraph.initBoard('JXGd19b800a-57a9-4303-b49a-8f5b7a5488f0', {boundingbox:[-5,5,5,-5], keepaspectratio:true, axis:true, showcopyright: false});
 *  col = 'blue';
 *  p = [];
 *  p.push(board.create('point',[-2, -1 ], {size: 5, strokeColor:col, fillColor:col}));
 *  p.push(board.create('point',[2, -1 ], {size: 5, strokeColor:'yellow', fillColor:'yellow'}));
 ...
 * <script type="text/javascript">
 *  (function () {
 *       let board = JXG2.JSXGraph.initBoard('JXGd19b800a-57a9-4303-b49a-8f5b7a5489f1')
 *
 *       // Add an image and use the group tools to manipulate it
 *       let urlImg = "https://jsxgraph.org/distrib/images/uccellino.jpg";
 *       let lowleft = [-2, -1]
 ...
 * </script><pre>
 */
export function createGroup(board, parents, attributes) {
    var attr = Type.copyAttributes(attributes, board.options, 'group'),
        g = new Group(board, attr.id, attr.name, parents, attr);
