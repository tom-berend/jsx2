...
export class Board extends Events {
     ...
     */
    public licenseText = `JSXGraph v${Version} \u00A9 jsxgraph.org`;
    ...
    generateId() {
        ...
        while (Type.exists(this.boards['jxgBoard' + r])) {
        ...
        if (keepaspectratio) {
            ...
            if (w >= h) {
                if (prev_w >= prev_h) {
                    ...
                    this.unitY = h / Math.abs(dx) * Math.sign(dy) / zoom_ratio;
                    this.unitX = this.unitY * ratio;
                }
            } else {
                if (prev_h > prev_w) {
                    ...
                    // Switch dominating interval
                    this.unitX = w / Math.abs(dy) * Math.sign(dx) * zoom_ratio;
                    this.unitY = this.unitX / ratio;
                }
            }
    ...
    initRenderer(containerName: string, dim: Dim, doc: any = null, rendererType: string = 'svg')/*: AbstractRenderer */ {
        ...
        switch (rendererType) {
            ...
            default:
                ...
                throw new Error(`Only SVG Renderer supported for now ${rendererType}`)
        }
            ...
            var alpha = 0,
                ...
                arclen = function (c, a, b) {
                    var cpxa = Numerics.D(c.X)(a),
                        ...
                        fa = Math.hypot(cpxa, cpya),
                        fb = Math.hypot(cpxb, cpyb),
                        fab = Math.hypot(cpxab, cpyab);
