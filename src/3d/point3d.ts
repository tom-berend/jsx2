...
import { GeometryElement3D } from "./element3d.js";
...
export class Point3D extends GeometryElement3D {
     ...
     * @private
     */
    coords3D = [0, 0, 0, 0];        // was just 'coords' but conflicts wit coords object
    initialCoords = [0, 0, 0, 0];
 ...
 */
    ...
    constructor(view, F, slide, attributes) {
        super(view, OBJECT_TYPE.POINT3D);
    ...
    updateCoords() {
        ...
        if (Type.isFunction(this.F)) {
            this.coords3D = Type.evaluate(this.F);
            if (this.coords3D.length === 3) {
                this.coords3D.unshift(1);
            }
        } else {
            if (this.F.length === 3) {
                this.coords3D[0] = 1;
            ...
            for (i = 0; i < this.F.length; i++) {
                // Attention: if F is array of numbers, coords3D may not be updated.
                // Otherwise, dragging will not work anymore.
                if (Type.isFunction(this.F[i])) {
                    this.coords3D[s + i] = Type.evaluate(this.F[i]);
                }
            }
        }
    ...
    initCoords() {
        ...
        this.initialCoords = this.coords3D.slice();
     ...
     */
    normalizeCoords() {
        if (Math.abs(this.coords3D[0]) > 1.e-14) {
            this.coords3D[1] /= this.coords3D[0];
            this.coords3D[2] /= this.coords3D[0];
            this.coords3D[3] /= this.coords3D[0];
            this.coords3D[0] = 1.0;
        }
        return this;
    }
     ...
     * @memberOf Point3D
     * @function
     * @param {Array} coords3D 3D coordinates. Either of the form [x,y,z] (Euclidean) or [w,x,y,z] (homogeneous).
    ...
    setPosition(coords, noevent) {
        var c = this.coords3D;
        // oc = this.coords3D.slice(); // Copy of original values
    ...
    updateTransform() {
        ...
        if (this === this.baseElement) {
            ...
            c = this.baseElement.coords3D;   // tbtb - assumed.  but maybe its a 2d.coords object?
         }
    ...
    update(drag) {
        ...
        if (
            ...
            if (this.view.isVerticalDrag()) {
                ...
                c3d = this.view.project2DTo3DVertical(this.element2D, this.coords3D);
            } else {
                // Drag the point in its xy plane
                foot = [1, 0, 0, this.coords3D[3]];
            ...
            if (c3d[0] !== 0) {
                ...
                if (res[1]) {
                    ...
                    this.element2D.coords.setCoordinates(
                        COORDS_BY.USER,
                        this.view.project3DTo2D(this.coords3D)
                    );
                }
                if (this.slide) {
                    this.coords3D = this.slide.projectCoords([1, this.X(), this.Y(), this.Z()], this.position);
                    this.element2D.coords.setCoordinates(
                        COORDS_BY.USER,
                        this.view.project3DTo2D(this.coords3D)
                    );
     ...
     */
    testIfFinite() {
        return Math.abs(this.coords3D[0]) > 1.e-12 ? true : false;
        // return Type.cmpArrays(this.coords3D, [0, 0, 0, 0]);
    }
    ...
    */
    moveTo(where, time, options) {
        ...
        var i,
            ...
            Y = where[1],
            Z = where[2],
            dX = this.coords3D[1] - X,
            dY = this.coords3D[2] - Y,
            dZ = this.coords3D[3] - Z,
            doneCallback = () => { },
            stepFun;
        ...
        if (
            ...
            // Math.abs(where.usrCoords[0] - this.coords3D.usrCoords[0]) > JSXMath.eps
        ) {
        ...
        return new Promise((resolve, reject) => {
            this.moveInterval = setInterval(() => {
                ...
                if (this.movePath.length === 0) {   // now shorter than previous test
                    ...
                    this.resolve();    // tbtb - was just 'resolve', did this ever work??
 ...
 *
 */
export function createPoint3D(board, parents, attributes) {
    //   parents[0]: view
    // followed by
    ...
    el = new Point3D(view, F, slide, attr);
    el.initCoords();
